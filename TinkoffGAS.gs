...
const OPENAPI_TOKEN = "t.h9Ta_KhnbtUsegQR40Y5hp1bZePXfKZy8K6jH5pXl8FE4_k-U7SBPFwPdQ2SbpEjgZmSjHj_RJ21P6cyFGgSXg"
...
function runRefresh() {
  for (i=0; i<43200; i++){
    ...
    if (flag == false) {
      SpreadsheetApp.getUi().ui.alert('cell IS_UPDATED == false');
      return;
    }
...
function checkIsUpdatedCell() {
  ...
  if (isUpdatedRange != null) {
    ...
    SpreadsheetApp.getUi().alert('You should specify the named range "IS_UPDATED" for using this function.');
  }
}
